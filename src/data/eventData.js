...
export function generateGoogleCalendarUrl(event) {
  ...
  const timeStr = typeof event.time === "object" ? (event.time.id || "") : (event.time || "");
  if (timeStr) {
  ...
  const titleVal = typeof event.title === "object" ? (event.title.id || "") : (event.title || "Agenda MKn UNISSULA");
  const descVal = typeof event.fullDescription === "object"
    ? (event.fullDescription.id || "")
    : (event.fullDescription || (typeof event.description === "object" ? (event.description.id || "") : (event.description || "")));
  const orgVal = typeof event.organizer === "object" ? (event.organizer.id || "") : (event.organizer || "MKn UNISSULA");
  const venueVal = typeof event.venue === "object" ? (event.venue.id || "") : (event.venue || "MKn UNISSULA");

  const title = encodeURIComponent(titleVal);
  const details = encodeURIComponent(`${descVal}\n\nPenyelenggara: ${orgVal}`);
  const location = encodeURIComponent(venueVal);
