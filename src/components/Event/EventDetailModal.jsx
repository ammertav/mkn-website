...
import { FiX, FiCalendar, FiClock, FiMapPin, FiUser, FiExternalLink } from "react-icons/fi";
        ...
        <div className="p-6 sm:p-8 space-y-6">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-gray-500 mt-1">
              {t({ id: "Diselenggarakan oleh:", en: "Organized by:" })}{" "}
              <span className="font-medium text-heading">{t(event.organizer)}</span>
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 p-4 bg-gray-50 rounded-xs border border-gray-200/80 text-xs sm:text-sm">
            <div className="flex items-start gap-3">
              ...
              <div>
                <span className="text-[11px] uppercase tracking-wider text-gray-500 font-semibold block">
                  {t({ id: "Hari & Tanggal", en: "Day & Date" })}
                </span>
                <span className="font-semibold text-heading">
                  {getIndoDayName(event.date, lang)}, {formatIndoDate(event.date, lang)}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                <span className="text-[11px] uppercase tracking-wider text-gray-500 font-semibold block">
                  {t({ id: "Waktu Pelaksanaan", en: "Event Time" })}
                </span>
                <span className="font-semibold text-heading">{t(event.time)}</span>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 sm:col-span-2">
              ...
              <div>
                <span className="text-[11px] uppercase tracking-wider text-gray-500 font-semibold block">
                  {t({ id: "Lokasi / Ruangan", en: "Location / Venue" })}
                </span>
                <span className="font-semibold text-heading">{t(event.venue)}</span>
              </div>
            </div>
            ...
            {event.speaker && (
              <div className="flex items-start gap-3 sm:col-span-2">
                ...
                <div>
                  <span className="text-[11px] uppercase tracking-wider text-gray-500 font-semibold block">
                    {t({ id: "Narasumber / Pengisi Acara", en: "Speaker / Resource Person" })}
                  </span>
                  <span className="font-semibold text-heading">{t(event.speaker)}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="space-y-2">
            <h3 className="text-xs uppercase tracking-wider font-bold text-heading border-b border-gray-200 pb-2">
              {t({ id: "Deskripsi Acara", en: "Event Description" })}
            </h3>
          ...
          {event.cp && (
            <div className="p-3 bg-gray-50 rounded-xs border border-gray-200 text-xs text-body">
              <span className="font-semibold text-heading">
                {t({ id: "Narahubung & Info Registrasi:", en: "Contact & Registration Info:" })}{" "}
              </span>
              <span>{event.cp}</span>
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-3 pt-2">
            {event.registrationUrl && (
              ...
              >
                <span>{t({ id: "Daftar / Akses Acara", en: "Register / Access Event" })}</span>
